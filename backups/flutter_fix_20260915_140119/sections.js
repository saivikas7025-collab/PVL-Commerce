...
// --- resolve the pg pool the same way the rest of the app does ---
let pool;
try {
  pool = require('../db');
  if (pool && pool.pool) pool = pool.pool;          // handles { pool } export
} catch (_) {
...
// ---------------------------------------------------------------
// GET /api/sections
// Top-level grid for the Categories tab.
// ---------------------------------------------------------------
...
// ---------------------------------------------------------------
// GET /api/sections/:section/categories
// Left-rail chips inside a section.
// ---------------------------------------------------------------
...
// Product grid for a section (optionally filtered by subcategory).
// ---------------------------------------------------------------
router.get('/sections/:section/products', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT p.id, p.name, p.unit, p.price, p.original_price,
             p.icon, p.bg_color, p.image_url,
             c.id   AS category_id,
